...
interface DramaHomeProps {
    trending: DramaSearchResponse;
            ...
            <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
                ...
                <form onSubmit={handleSearch} className="hidden flex-1 sm:mx-8 sm:block sm:max-w-md">
                    <div className="relative">
                        ...
                        />
                        <button type="submit" className="absolute right-3 top-1/2 -translate-y-1/2 text-theme-secondary hover:text-primary">
...
function DramaGrid({ title, items }: { title: string; items: DramaResult[] }) {
    ...
    return (
        <>
            ...
            <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
                    ...
                    <section className="mb-10">
                        ...
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            {continueWatching.map((item) => {
                                const displayTitle = item.anime_title ||
                                    item.anime_id
                                        .replace(/-\d+$/, '')
                                        .replace(/^tv\/watch-/, '')
                                        .replace(/-/g, ' ')
                                        .replace(/\b\w/g, (c) => c.toUpperCase());
                                const progressPercent = Math.min((item.progress_seconds / 3600) * 100, 95);
                                const watchedMin = Math.floor(item.progress_seconds / 60);

                                return (
                                    <Link
                                        key={item.id}
                                        href={route('drama.watch', { id: item.anime_id, episodeId: item.episode_id })}
                                    ...
                                    >
                                        <div className="relative aspect-[16/7] w-full overflow-hidden bg-input">
                                            {item.anime_image ? (
                                                <img src={item.anime_image} alt={displayTitle} className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" />
                                            ) : (
                                            ...
                                            <div className="absolute inset-0 flex items-center justify-center opacity-0 transition-opacity duration-200 group-hover:opacity-100">
                                                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-secondary/90 shadow-lg">
                                                    <svg className="ml-0.5 h-4 w-4 text-base" fill="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <div className="px-3 pb-3 pt-2">
                                            <p className="truncate text-sm font-semibold text-primary">{displayTitle}</p>
                                            <p className="mt-0.5 text-xs text-theme-secondary">
                                                Episode {item.episode_number} · {watchedMin}m watched
                                            </p>
                ...
                <DramaGrid title="Trending Dramas" items={trending?.results || []} />
            </div>
        </>
    );
}

export default function DramaHome(props: DramaHomeProps) {
    const { auth } = usePage().props as { auth?: { user?: unknown } };

    if (auth?.user) {
        return (
            <AuthenticatedLayout>
                <DramaHomeContent {...props} />
            </AuthenticatedLayout>
        );
    }
